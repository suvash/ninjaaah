...
//-------------------------------------------------------------------------------------
void RandGen::generateMap(struct map *arena, int dim_x, int dim_y, int room_min_x, int room_min_y, int room_max_area, int door_cnt, bool furniture_enable, int desk_cnt, int chair_cnt, int shelf_cnt, int painting_cnt)
{
    //Fill structures
...
void RandGen::addBlackTile(tile pos, struct map *arena)
{
	arena->blackTile[pos.x][pos.y] = 1;
}
void RandGen::addWall(tile len, tile pos, struct map *arena)
{
	tile p;
	if (len.x==0)
	{
